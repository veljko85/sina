...
var createScene = function () {
  ...
  function changePosAndScaleFence(valueToCount, activeFence) {
    ...
    if (
      fencesArr[activeFence].type == "sina180180" ||
      fencesArr[activeFence].type == "sina18090"
    ) {
  ...
  //MAIN POST MESH //////////////////////////////////////////////////////////////////////////
  BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "mainPostSina8.glb").then(
    (result) => {
  ...
  var createRightFence = (posX, posZ, rotY, type) =>
    BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "sinaCombo23.glb").then(
      (result) => {
        ...
        function toActiveFence() {
          ...
          if (fencesArr[activeFence].type == "sina180180")
            setActivnesStyle(changeFence, 0, 0, "active-text-color");
          if (fencesArr[activeFence].type == "sina90180")
            setActivnesStyle(changeFence, 0, 1, "active-text-color");
          if (fencesArr[activeFence].type == "sinaKosaDesna")
            setActivnesStyle(changeFence, 0, 2, "active-text-color");
          if (fencesArr[activeFence].type == "sinaKosaLeva")
            setActivnesStyle(changeFence, 0, 3, "active-text-color");
          if (fencesArr[activeFence].type == "sina18090")
        ...
        fakeFence.position = new BABYLON.Vector3(
          getAbsPosX(result.meshes[9]) + 0.03,
          0.9,
        ...
        result.meshes[9].addChild(fakeFence);

        //POST CAP
        let rightPostCap = result.meshes[1];
        ...
        //METAL PART TO HOLD WOOD POST
        let rightMetalPart = result.meshes[15];
        rightMetalPart.material = rootMat;
        metalParts.push(rightMetalPart);
        rightMetalParts.push(rightMetalPart);

        //SMALL METAL PART TO HOLD WOOD POST
        let rightSmallMetalPartFront = result.meshes[8];
        let rightSmallMetalPartBack = result.meshes[6];
        let rightSmallMetalPartBoth = result.meshes[7];
        ...
        //WOOD POST
        let rightWoodPost = result.meshes[14];
        ...
        //STRUMANKER
        let rightStrVord = result.meshes[4];
        ...
        rightPostCap.addChild(result.meshes[10]);
        ...
        let sina180180 = result.meshes[9];
        sina180180Arr.push(sina180180);
        sina180180.isVisible = false;
        let sina90180 = result.meshes[10];
        sina90180Arr.push(sina90180);
        sina90180.isVisible = false;
        let sina18090 = result.meshes[11];
        sina18090Arr.push(sina18090);
        sina18090.isVisible = false;
        let sinaKosaLeva = result.meshes[12];
        sinaKosaLevaArr.push(sinaKosaLeva);
        sinaKosaLeva.isVisible = false;
        let sinaKosaDesna = result.meshes[13];
        sinaKosaDesnaArr.push(sinaKosaDesna);
        sinaKosaDesna.isVisible = false;
        ...
        if (type == "sina180180") sina180180.isVisible = true;

        if (type == "sina90180") {
          sina90180.isVisible = true;
          setSmallerFenceSize(
            rightPost,
            rightPostCap,
            foundationRight,
        ...
        if (type == "sinaKosaLeva") {
          sinaKosaLeva.isVisible = true;
          setSmallerFenceSize(
            rightPost,
            rightPostCap,
            foundationRight,
        ...
        if (befePfostenSize == 1) {
          ...
          if (type == "sina18090" || type == "sinaKosaDesna") {
        ...
        if (type == "sina180180" || type == "sina18090") {
          fenceSizeObj = 180;
        }
  ...
  function handleTabActivnes() {
    if (!document.hidden) {
      createRightFence(0.99, 0, 0, "sina180180");
    ...
    for (let i = 0; i < fencesArr.length; i++) {
      if (
        ...
        for (let j = 0; j < fencesArr[i].children.length; j++) {
          if (
            fencesArr[fencesArr[i].children[j]].type != "sina18090" &&
            fencesArr[fencesArr[i].children[j]].type != "sinaKosaLeva"
          ) {
  ...
  function changeFenceFunction(c, d, h) {
    ...
    if (d == "sina180180") {
      sina180180Arr[h].isVisible = true;
    }
    if (d == "sina90180") {
      sina90180Arr[h].isVisible = true;
    }
    if (d == "sinaKosaDesna") {
      sinaKosaDesnaArr[h].isVisible = true;
    }
    if (d == "sinaKosaLeva") {
      sinaKosaLevaArr[h].isVisible = true;
    }
    if (d == "sina18090") {
      sina18090Arr[h].isVisible = true;
    }
    ...
    if (d == "sina18090") {
      ...
      for (let i = 0; i < fencesArr[h].children.length; i++) {
        if (
          fencesArr[fencesArr[h].children[i]].type != "sina18090" &&
          fencesArr[fencesArr[h].children[i]].type != "sinaKosaLeva"
        ) {
          childType += 1;
        }
      }
  ...
  changeFence[0].addEventListener("click", () => {
    changeFenceFunction(180, "sina180180", activeFence);
  });
  changeFence[1].addEventListener("click", () => {
    changeFenceFunction(90, "sina90180", activeFence);
  });
  changeFence[2].addEventListener("click", () => {
    changeFenceFunction(90, "sinaKosaDesna", activeFence);
  });
  changeFence[3].addEventListener("click", () => {
    changeFenceFunction(90, "sinaKosaLeva", activeFence);
  });
  changeFence[4].addEventListener("click", () => {
    changeFenceFunction(180, "sina18090", activeFence);
  });
    ...
    if (fencesArr[activeFence].type == "sina180180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(180, "sina180180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "sina90180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "sina90180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "sinaKosaDesna") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "sinaKosaDesna", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "sinaKosaLeva") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "sinaKosaLeva", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "sina18090") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "sina18090", i);
        }
      }
    }
  ...
  function deleteFence(a) {
    ...
    if (
      ...
      (fencesArr[fencesArr[a].parent].type == "sina18090" ||
        fencesArr[fencesArr[a].parent].type == "sinaKosaDesna")
    ) {
      let childTypee = 0;
